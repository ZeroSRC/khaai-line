...
⚙️ (settings) ──► /shop/[shopId]/settings
...
**Path:** `/shop/[shopId]/settings` — เข้าจากไอคอนเฟือง ⚙️ บน Dashboard

```
Settings hub — เมนูแบ่งเป็น section
    │
    ...
    │             │
    │             └── ปุ่ม "เชิญสมาชิกใหม่" (เฉพาะ owner) ──► /settings/members/invite
    │                     ├── คัดลอกลิงก์เชิญ ( {origin}/shop/[slug]/join )
    │                     └── เพิ่มด้วย LINE User ID + เลือก role (staff/finance)
    │
    ├── [ทั่วไป]
    │     └── ภาษา ──► /settings/language
    │             └── เลือก ไทย 🇹🇭 / English 🇬🇧
    │                 บันทึกลง localStorage (khaai_lang) ผ่าน langStore (zustand persist)
    │                 ทั้งแอปเปลี่ยนภาษาทันที (useT() hook อ่านจาก store)
    │
    └── [บัญชี]
          └── เปลี่ยนร้านค้า ──► ล้าง store + localStorage ──► /
```

**i18n:** dictionary อยู่ที่ `src/lib/i18n.ts` (th เป็นภาษาต้นทาง, en mirror)
ทุกหน้าเรียก `const t = useT()` แล้ว `t('section.key')` — รองรับ interpolation `t('key', { n: 3 })`
