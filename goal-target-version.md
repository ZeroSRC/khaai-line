...
- [x] กราฟยอดขายรายวัน
- [x] ~~Export CSV~~ — เอาออกแล้ว 2026-07-18 (ผู้ใช้ขอ) โค้ดถูกลบทิ้ง ไม่ใช่ซ่อน
...
- [x] **COGS ใช้ทุนปัจจุบัน (2026-07-18)** — เปลี่ยนจาก `sale_items.unit_cost` (snapshot) เป็น `products.cost_price` (สด) แก้ราคาซื้อย้อนหลัง → กำไรบิลขายเก่า/รายงานเดือนที่ปิดแล้วขยับตามทันที (ตั้งใจ ผู้ใช้ยืนยันแล้ว) รายละเอียด: `docs/user-flow.md` § รายงาน
- [x] **เลือกช่วงวันที่กำหนดเอง (2026-07-18)** — เพิ่ม pill "รายเดือน/กำหนดเอง" ทุกการ์ด+กราฟ+CSV คำนวณจาก [dateStart, dateEnd] เดียวกัน ไม่กระทบ `MonthFilter` ที่หน้าอื่นใช้ร่วม
...
- [ ] ผูก tracking API ของขนส่งที่เหลือ (ตอนนี้มีแค่ **Flash**) — Kerry / ไปรษณีย์ไทย / SPX / Best
...
- [x] **J&T (2026-07-19) — พยายามแล้ว ทำไม่ได้ ไม่ใช่ "ยังไม่ทำ"**: เว็บ tracking สาธารณะของ J&T มี slide-puzzle
      captcha ป้องกันบอท ไม่มี token คงที่ให้เรียกอัตโนมัติได้ (ต่างจาก Flash ที่ endpoint เปิดเผยตรงๆ) ทดสอบ
      token จาก network tab จริงแล้วก็ยังโดนปัด `code:20005` (verifyCode ผิด/หมดอายุ) — **ไม่ทำระบบแก้ captcha
      อัตโนมัติ** (ข้าม anti-bot ที่ J&T ตั้งใจใส่ไว้) ต้องขอ API key จาก J&T merchant program แทนถึงจะใช้งานได้จริง
      โครงพร้อมไว้แล้วที่ `src/app/api/jt-track/route.ts` แต่ยังไม่ต่อเข้า UI

> ตอนนี้ปุ่ม sync สถานะทำงานเฉพาะ Flash (`syncTracking()` ใน `shipments/page.tsx`) ร้านที่ใช้เจ้าอื่นต้องกดอัปเดตสถานะเองทุกชิ้น
...
> ⚠️ **บั๊กใหม่จาก soft-delete migration (พบ 2026-07-18):** เพิ่มสมาชิกด้วย LINE UID ไม่ได้ ถ้า UID นั้นเคยถูกลบ (soft-delete) ออกจากร้านมาก่อน
> สาเหตุ: `shop_members` มี `unique(shop_id, line_uid)` แบบเดิม ไม่ได้แปลงเป็น partial unique index ตอนทำ soft-delete (พลาดไป —
...
> ⚠️ **กำไรอ้างอิงทุนปัจจุบันเสมอ (ไม่ snapshot แล้ว ตั้งแต่ 2026-07-18)** — ใช้ `products.cost_price` สด ไม่ใช่ `sale_items.unit_cost` เวลา AI สรุปกำไรย้อนหลัง ต้อง query ด้วยสูตรเดียวกับหน้า reports (ดู `docs/user-flow.md` § รายงาน) ไม่งั้นเลขไม่ตรงกัน
