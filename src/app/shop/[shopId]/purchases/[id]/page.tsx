    ...
    <div className="pb-10">
      <div className="px-4 pt-12 pb-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-base font-bold text-gray-900">รายละเอียดการซื้อ</h1>
      ...
      <div className="px-4 space-y-3">
        ...
        <div className="bg-red-500 rounded-3xl p-6 shadow-[0_8px_32px_rgba(239,68,68,0.25)]">
          <p className="text-white/70 text-xs font-semibold mb-1">ยอดซื้อรวม</p>
          ...
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/20">
            ...
            {purchase.slip_url && <span className="text-[11px] bg-white/20 text-white px-2.5 py-1 rounded-full">มีสลิป</span>}
          </div>
        </div>
            ...
            {items.map((item, i) => (
              <div key={item.id} className={`flex items-center gap-3 px-4 py-3 ${i > 0 ? 'border-t border-gray-50' : ''}`}>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-900">{item.product?.name ?? 'สินค้า'}</p>
